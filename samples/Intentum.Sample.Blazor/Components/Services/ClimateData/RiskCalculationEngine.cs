...
public sealed class RiskCalculationEngine
{
        ...
        RiskInput input, CancellationToken ct = default)
    {
        ...
        return new RiskAssessment
        {
            ...
            OverallRisk = (physicalScore * 0.6 + transitionScore * 0.4),
    ...
    private double CalculatePhysicalRisk(ClimateProjection? projection, WriCountryRisk? wri, RiskInput input)
    {
        ...
        {
            var tempAnomaly = projection.AvgTempMax - 30;
            score += Math.Clamp(tempAnomaly / 10.0, 0, 1) * 0.25;
        }

        if (projection != null)
        {
            var precipChange = projection.AvgPrecipitation;
            score += Math.Clamp(Math.Abs(precipChange - 2.0) / 5.0, 0, 1) * 0.2;
        }
        ...
        if (wri != null)
        {
            score += (wri.WaterStress / 5.0) * 0.25;
        }

        if (wri != null)
        {
            score += (wri.FloodRisk / 5.0) * 0.15;
        }
