...
// Dedicated HttpClient for SSE: no timeout so long-lived stream is not dropped
builder.Services.AddHttpClient("Intentum.SSE", c => c.Timeout = Timeout.InfiniteTimeSpan);
// HttpClient for Blazor components: same-origin API calls (BaseAddress from request)
...
// Intentum: infer + rate limit + persist
...
// Sustainability timeline simulation
