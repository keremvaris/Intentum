...
**Current state:** The Sample Web has a **Playground** section in the UI (Examples → Playground): enter events (or pick a preset), check models (Default, Mock), click **Compare**, see a table per model. API: `POST /api/intent/playground/compare`.

The Playground is available in the Blazor sample UI. You can enter events (or pick a preset), choose model names (e.g. Default, Mock), click "Compare", and see a table with intent and decision per model — no need to call the API by hand.
...
| **Playground** | Compare different models on the same events (API + UI in Blazor sample). |
